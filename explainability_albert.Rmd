...
Die Interpretierbaren Modelle werden grob in die zwei verschiedenen Kategorien (a) Regression und (b) Klassifikation gegliedert. Im folgenden Abschnitt werden ein paar der häufigsten Modelle kurz besprochen und mit einem Code Beispiel gezeigt, was mit unserem ausgewählten Datensatz interpretiert werden könnte. 
2.1 Lineare Regression
```{r Lineare Regression Code Beispiel, message=FALSE, warning=FALSE}

```

2.2 Logistische Regression
```{r Logistische Regression Code Beispiel, message=FALSE, warning=FALSE}

```


2.3 GLM und GAM
```{r GLM & GAM Code Beispiel, message=FALSE, warning=FALSE}

```


2.4 Entscheidungsbäume
```{r Decision Trees Code Beispiel, message=FALSE, warning=FALSE}
...
```{r Decision Rules Code Beispiel, message=FALSE, warning=FALSE}

```

2.6 RuleFit
```{r RuleFit  Code Beispiel, message=FALSE, warning=FALSE}
...
Unter modell-agnostische Methoden versteht man die Trennung vom ML Modell von deren Interpretation. Der größe Vorteil davon ist, die dadurch gewonne Flexibilität. Wenn wir uns das in der Praxis anschauen wissen wir zu Beginn nicht welches das beste ML Modell ist. Wenn nun ein model-spezifischer Ansatz gewählt wurde um die Ergebnisse zu intpretieren, dann können die Modelle nicht immer leicht verglichen werden. Wenn modell-agnostische Methoden für die Interpretation verwendet werden, dann vereinfacht dies die Vergleichbarkeit über verschiedene ML Modelle. Drei wichtige Aspekte für modell-agnostische Interpretationen sind (Ribeiro, Singh, and Guestrin 2016):
(a) Modellflexibilität: Wieviele verschiedene ML Ansätze können damit erklärt werden?
(b) Interpretationsflexibilität: Wie frei sind wir in der Entscheidung ein Modell zu Interpretieren? 
(c) Representationsflexibilität: Wie unterschiedlich sind die Features von dem ML Modell zu den Features der Interpretation?
...
Mit einem Partial Dependence Plot können wir heraus finden wie die Beziehung der verschiedenen Variablen aussieht, in anderen Worten können wir feststellen, ob die Beziehung linear, monoton oder komplexer ist. Der Vorteil der PDP ist, dass die Interpretation von den Modellen recht intuitiv ist und dadurch für die viele Leute recht einfach zu verstehen. Im Gegenzug dazu sind die Nachteile größer. Unteranderem können nur eine begrenzte Anzahl an Features dargestellt werden, weil der PDP immer 2 Dimensional ist. Ein anderen Nachteil ist, dass Hintergrundinformationen verloren geht. Dies könnte unter anderem die nicht in betracht nahme von Korrelationen mit anderen Features, die nicht für den PDP herangezogen wurden. 
```{r Partial Dependence Plot Code Beispiel, message=FALSE, warning=FALSE}

```

3.2 Local Surrogate 
Die Grundidee vom "Local interpretabke model-agnoistic" (LIME) Modell ist, dass wir den Fokus auf das ML Modell legen und nicht auf die Trainingsdaten. Nun Versuchen wir Vorhersagen mit unserem Modell zu treffen und schauen uns immer die Resultate an. Im nächsten Schritt verändern wir die Daten mit denen wir die Vorhersage gemacht haben und schauen uns die unterschiede zu der vorherigen Vorhersage an. In anderen Worten versuchen wir heraus zu finden, welche Auswirkungen die Änderungen an den Input Daten an den Output Daten hat. 

```{r Local Surrogate Code Beispiel, message=FALSE, warning=FALSE}

```

3.3 Shapley Values

```{r Shapley Values Code Beispiel, message=FALSE, warning=FALSE}

```

3.4 SHAP
```{r SHAP Code Beispiel, message=FALSE, warning=FALSE}
